...
if __name__ == "__main__":
    ...
    mode = None
    motor = None
    print("Choose a mode: tideHeight, neapSpring, ebbFlow")
    while mode not in ["tideheight", "neapspring", "ebbflow"]:

        mode = input("Mode: ").strip().lower()
        if mode == "tideheight":
            motor = tideHeight
        elif mode == "neapspring":
            motor = neapSpring
        elif mode == "ebbflow":
            motor = ebbFlow
    ...
    print(f"Mode: {mode}")
    while True:
        print("Enter a position between 0 and 200...")
        try:
            target = int(input("Position: "))
            if target < 0 or target > STEPS:
                print(f"Please enter a value between 0 and {STEPS}.")
                continue
        except ValueError:
